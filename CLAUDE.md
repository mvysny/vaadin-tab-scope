...
- **`testapp/`** — the runnable Vaadin-Boot demo. Package `testapp`. Owns the two SPI registration files, `ApplicationServiceInitListener`, and (demoing the tab-close beacon hook) a `@WebServlet` `TabScopeBeaconServlet` + `@PreserveOnRefresh` `PreserveView`.
...
1. **`TabScope`** — holds per-tab state keyed by `ExtendedClientDetails.windowName`. The scope map lives on `VaadinSession` under attribute `"tab-scopes"`. A single `TabScope` may transiently have 0 or 2 UIs attached (during page reload the old UI detaches before the new one attaches), so orphan detection uses a **60-second grace period** (`CLEANUP_DURATION_MS`) rather than killing the scope the moment UI count hits 0. Do not shorten this without considering reload races. When a scope orphans, a one-shot reap is armed on a shared daemon `ScheduledExecutorService` (the scheduled reaper, on by default; disable via the public `TabScope.scheduledReapEnabled = false` to drop the reaper thread and ride Vaadin's default closing) so a sole last tab is destroyed within the grace period even with no further request; it runs off-request via `session.access` and is cancelled on UI reattach. `TabScope.onUnloadBeacon(UI)` + `TabScopeServerRpcHandler`/`TabScopeUidlRequestHandler` + `installTabCloseBeacon` (the tab-close beacon hook) let an app route the tab-close beacon in, starting the clock without closing a `@PreserveOnRefresh` UI. See INTERNALS.md ("Cleanup").
